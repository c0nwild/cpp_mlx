...
int MLX90640::DumpEE(std::vector<std::uint16_t> &eeData) {
  ...
  //                         MLX90640_EEPROM_DUMP_NUM, eeData);
  return mlx_i2c_
      ->read_to_buffer(MLX90640_EEPROM_START_ADDRESS, eeData,
                       MLX90640_EEPROM_DUMP_NUM)
      .value();
}

int MLX90640::SynchFrame() {
  uint16_t dataReady = 0;
  uint16_t statusRegister;
  int error = 1;

  if (!mlx_i2c_->write_word(MLX90640_STATUS_REG, MLX90640_INIT_STATUS_VALUE)) {
    return MLX90640_I2C_NACK_ERROR;
  }

  while (dataReady == 0) {
    auto i2c_read_retval = mlx_i2c_->read_word(MLX90640_STATUS_REG);
    if (!i2c_read_retval) {
      return MLX90640_I2C_NACK_ERROR;
    }
    // dataReady = statusRegister & 0x0008;
    dataReady = MLX90640_GET_DATA_READY(i2c_read_retval.value());
  }

  return MLX90640_NO_ERROR;
}

int MLX90640::TriggerMeasurement() {
  int error = 1;
  std::optional<std::uint16_t> ctrlReg;
  bool i2c_retval;

  // error = MLX90640_I2CRead(slaveAddr, MLX90640_CTRL_REG, 1, &ctrlReg);
  ctrlReg = mlx_i2c_->read_word(MLX90640_CTRL_REG);

  if (!ctrlReg) {
    return error;
  }

  ctrlReg.value() |= MLX90640_CTRL_TRIG_READY_MASK;
  // error = MLX90640_I2CWrite(slaveAddr, MLX90640_CTRL_REG, ctrlReg);
  i2c_retval = mlx_i2c_->write_word(MLX90640_CTRL_REG, ctrlReg.value());
  ...
  if ((ctrlReg.value() & MLX90640_CTRL_TRIG_READY_MASK) != 0) {
...
int MLX90640::GetFrameData(std::vector<std::uint16_t> &frameData) {
  uint16_t dataReady = 0;
  uint16_t controlRegister1;
  std::optional<std::uint16_t> statusRegister;
  int error = 1;
  uint16_t data[64];
  uint8_t cnt = 0;
  bool i2c_retval;
  std::optional<std::uint16_t> i2c_buffer_read_retval;

  while (dataReady == 0) {
    statusRegister = mlx_i2c_->read_word(MLX90640_STATUS_REG);
    if (!statusRegister) {
      return MLX90640_I2C_NACK_ERROR;
    }
    // dataReady = statusRegister & 0x0008;
    dataReady = MLX90640_GET_DATA_READY(statusRegister.value());
  }

  i2c_retval =
      mlx_i2c_->write_word(MLX90640_STATUS_REG, MLX90640_INIT_STATUS_VALUE);
  if (!i2c_retval) {
    return MLX90640_I2C_NACK_ERROR;
  }

  i2c_buffer_read_retval = mlx_i2c_->read_to_buffer(
      MLX90640_PIXEL_DATA_START_ADDRESS, frameData, MLX90640_PIXEL_NUM);
  if (error != MLX90640_NO_ERROR) {
    return MLX90640_I2C_NACK_ERROR;
  }

  error = MLX90640_I2CRead(slaveAddr, MLX90640_AUX_DATA_START_ADDRESS,
                           MLX90640_AUX_NUM, data);
  if (error != MLX90640_NO_ERROR) {
    return error;
  }
...
int MLX90640::ExtractParameters(std::vector<std::uint16_t> &eeData,
...
int MLX90640::GetCurResolution() {
  ...
  resolutionRAM = (controlRegister1 & ~MLX90640_CTRL_RESOLUTION_MASK) >>
...
int MLX90640::GetRefreshRate() {
  ...
  refreshRate = (controlRegister1 & ~MLX90640_CTRL_REFRESH_MASK) >>
...
int MLX90640::GetCurMode() {
  ...
  modeRAM = (controlRegister1 & MLX90640_CTRL_MEAS_MODE_MASK) >>
...
void MLX90640::CalculateTo(uint16_t *frameData, const paramsMLX90640 *params,
...
void MLX90640::GetImage(uint16_t *frameData, const paramsMLX90640 *params,
                        float *result) {
  float vdd;
  float ta;
  float gain;
...
float MLX90640::GetVdd(uint16_t *frameData, const paramsMLX90640 *params) {
  float vdd;
  float resolutionCorrection;

  uint16_t resolutionRAM;

  resolutionRAM = (frameData[832] & ~MLX90640_CTRL_RESOLUTION_MASK) >>
                  MLX90640_CTRL_RESOLUTION_SHIFT;
...
float MLX90640::GetTa(uint16_t *frameData, const paramsMLX90640 *params) {
  ...
  ptatArt =
      (ptat / (ptat * params->alphaPTAT + (int16_t)frameData[768])) * POW2(18);
...
void MLX90640::ExtractVDDParameters(std::vector<std::uint16_t> &eeData,
                                    paramsMLX90640 *mlx90640) {
  int8_t kVdd;
  int16_t vdd25;
...
void MLX90640::ExtractPTATParameters(std::vector<std::uint16_t> &eeData,
                                     paramsMLX90640 *mlx90640) {
  float KvPTAT;
  float KtPTAT;
  int16_t vPTAT25;
...
void MLX90640::ExtractGainParameters(std::vector<std::uint16_t> &eeData,
                                     paramsMLX90640 *mlx90640) {
...
void MLX90640::ExtractTgcParameters(std::vector<std::uint16_t> &eeData,
                                    paramsMLX90640 *mlx90640) {
...
void MLX90640::ExtractResolutionParameters(std::vector<std::uint16_t> &eeData,
                                           paramsMLX90640 *mlx90640) {
...
void MLX90640::ExtractKsTaParameters(std::vector<std::uint16_t> &eeData,
                                     paramsMLX90640 *mlx90640) {
...
void MLX90640::ExtractKsToParameters(std::vector<std::uint16_t> &eeData,
                                     paramsMLX90640 *mlx90640) {
  int32_t KsToScale;
  int8_t step;
...
void MLX90640::ExtractAlphaParameters(std::vector<std::uint16_t> &eeData,
                                      paramsMLX90640 *mlx90640) {
  int accRow[24];
  int accColumn[32];
  int p = 0;
...
void MLX90640::ExtractOffsetParameters(std::vector<std::uint16_t> &eeData,
                                       paramsMLX90640 *mlx90640) {
  int occRow[24];
  int occColumn[32];
  int p = 0;
...
void MLX90640::ExtractKtaPixelParameters(std::vector<std::uint16_t> &eeData,
                                         paramsMLX90640 *mlx90640) {
  int p = 0;
  int8_t KtaRC[4];
  uint8_t ktaScale1;
...
void MLX90640::ExtractKvPixelParameters(std::vector<std::uint16_t> &eeData,
                                        paramsMLX90640 *mlx90640) {
  int p = 0;
  int8_t KvT[4];
  int8_t KvRoCo;
...
void MLX90640::ExtractCPParameters(std::vector<std::uint16_t> &eeData,
                                   paramsMLX90640 *mlx90640) {
  float alphaSP[2];
  int16_t offsetSP[2];
  float cpKv;
...
void MLX90640::ExtractCILCParameters(std::vector<std::uint16_t> &eeData,
                                     paramsMLX90640 *mlx90640) {
  float ilChessC[3];
...
int MLX90640::ExtractDeviatingPixels(std::vector<std::uint16_t> &eeData,
                                     paramsMLX90640 *mlx90640) {
  uint16_t pixCnt = 0;
...
int MLX90640::IsPixelBad(uint16_t pixel, paramsMLX90640 *params) {
  for (int i = 0; i < 5; i++) {
